...
// ═══════════════════════════════════════════════════════════════════
//  DEBUG OVERLAY — activé uniquement avec ?debug=1 dans l'URL.
//  Compatible iOS Safari (pas de PerformanceObserver longtask).
//  Affiche en direct : taps, appels réseau (fetch) + durée, gels du thread,
//  et changements de question — pour localiser EXACTEMENT le délai ressenti.
// ═══════════════════════════════════════════════════════════════════
(function initDebugOverlay() {
    try {
        ...
        box.id = '__dbg';
        box.style.cssText = [
            'position:fixed', 'top:64px', 'left:6px', 'right:6px', 'z-index:99999',
            'background:rgba(0,0,0,0.82)', 'color:#0f0', 'font:600 11px/1.35 monospace',
            'padding:8px 10px', 'border:1px solid #0a0', 'border-radius:8px',
        ...
        const log = (msg, color) => {
            ...
            lines.push((color ? `<span style="color:${color}">` : '') + `[${ts}s] ${msg}` + (color ? '</span>' : ''));
        ...
        const ready = () => {
            ...
            log('DEBUG ON — reproduis le bug', '#ff0');
        };
        if (document.body) ready(); else document.addEventListener('DOMContentLoaded', ready);

        // 1) Taps sur les options du questionnaire
        document.addEventListener('click', (e) => {
            const card = e.target?.closest?.('.option-card, .btn-primary, .btn-secondary');
            if (card) {
                const label = (card.querySelector?.('.option-label')?.textContent || card.textContent || '').trim().slice(0, 22);
                log('👆 TAP: ' + label, '#6cf');
            }
        }, true);

        // 2) Interception des appels réseau (fetch) + durée
        const _fetch = window.fetch;
        window.fetch = function (...args) {
            let url = '';
            try { url = (typeof args[0] === 'string' ? args[0] : args[0]?.url) || ''; } catch (e) {}
            const short = url.replace(/^https?:\/\/[^/]+/, '').slice(0, 46);
            const start = performance.now();
            log('🌐 fetch → ' + short, '#fa0');
            return _fetch.apply(this, args).then(r => {
                log('✅ ' + Math.round(performance.now() - start) + 'ms ← ' + short, '#fa0');
                return r;
            }).catch(err => {
                log('❌ fetch FAIL ' + short, '#f44');
                throw err;
            });
        };

        // 3) Détection des GELS du thread (compatible iOS) — boucle rAF :
        //    si un frame met > 70ms, le thread a été bloqué pendant ce temps.
        let last = performance.now();
        const tick = () => {
            const now = performance.now();
            const delta = now - last;
            if (delta > 70) log('🧊 GEL ' + Math.round(delta) + 'ms (thread bloqué)', '#f55');
            last = now;
            requestAnimationFrame(tick);
        };
        requestAnimationFrame(tick);

        // 4) Changement de question (contenu du conteneur)
        const startObs = () => {
            const c = document.getElementById('question-container');
            if (!c) { setTimeout(startObs, 300); return; }
            new MutationObserver(() => {
                const h2 = c.querySelector('h2')?.textContent?.slice(0, 26) || '(vide)';
                log('🎬 QUESTION: ' + h2, '#9f9');
            }).observe(c, { childList: true });
        };
        startObs();
    } catch (e) { /* debug ne doit jamais casser l'app */ }
})();
