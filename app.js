...
import { historyService, ratingsService, watchlistService, preferencesService } from './services/supabase.js?v=14';
...
const App = {
    ...
    async init() {
        ...
        if (duoSessionId) {
            ...
            try {
                const { duoSessionService } = await import('./services/supabase.js?v=14');
            ...
            if (store.duoMode && store.duoMerged) {
                try {
                    ...
                    if (store._duoSessionId) {
                        import('./services/supabase.js?v=14').then(({ duoSessionService }) => {
    ...
    _showSaveGate(feature) {
        ...
        document.getElementById('save-gate-signup').onclick = () => {
            overlay.remove();
            import('./modules/auth.js?v=40').then(m => m.authUI.showModal('signup'));
        };
        document.getElementById('save-gate-signin').onclick = () => {
            overlay.remove();
            import('./modules/auth.js?v=40').then(m => m.authUI.showModal('signin'));
        };
    ...
    // ── Profil cinéphile ──
    async showProfile() {
        const { authUI } = await import('./modules/auth.js?v=40');
        authUI.showHistory();
    },
    ...
    async _changePassword() {
        ...
        try {
            const { authService } = await import('./services/supabase.js?v=14');
