...
const App = {
    ...
    _showSaveGate(feature) {
        ...
        document.getElementById('save-gate-signup').onclick = () => {
            overlay.remove();
            import('./modules/auth.js?v=38').then(m => m.authUI.showModal('signup'));
        };
        document.getElementById('save-gate-signin').onclick = () => {
            overlay.remove();
            import('./modules/auth.js?v=38').then(m => m.authUI.showModal('signin'));
        };
    ...
    // ── Profil cinéphile ──
    async showProfile() {
        const { authUI } = await import('./modules/auth.js?v=38');
        authUI.showHistory();
    },
