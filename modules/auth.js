...
import { t, applyTranslations } from '../config/i18n.js?v=354';
    ...
    async showForgotPassword() {
        ...
        wrap.innerHTML = `
            <div style="padding:0.5rem 0">
                ...
                <p style="font-size:0.82rem;color:rgba(255,255,255,0.45);margin-bottom:1.25rem">Entre ton email, on t'envoie un lien pour choisir un nouveau mot de passe.</p>
                <input type="email" id="forgot-email" placeholder="Email" style="width:100%;padding:0.8rem 1rem;background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.15);border-radius:10px;color:#fff;font-size:16px;font-family:inherit;outline:none;box-sizing:border-box;margin-bottom:0.75rem">
                <p id="forgot-msg" style="font-size:0.85rem;color:#46d369;display:none;margin-bottom:0.75rem;line-height:1.5"></p>
                <button id="btn-forgot-send" class="btn-primary" style="width:100%">Envoyer le lien</button>
            </div>
        `;
        ...
        document.getElementById('btn-forgot-send')?.addEventListener('click', async () => {
            ...
            try {
                ...
                if (msg) { msg.textContent = '📩 Lien envoyé ! Ouvre ton email (et tes spams) et clique sur le lien pour choisir un nouveau mot de passe.'; msg.style.display = 'block'; }
                btn.style.display = 'none';
            } catch(err) {
                if (msg) { msg.textContent = "Impossible d'envoyer le lien. Vérifie l'adresse."; msg.style.color = '#E50914'; msg.style.display = 'block'; }
